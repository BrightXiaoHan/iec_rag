...
OPENAI_COMPATIBLE_BASE_URL = os.getenv("OPENAI_COMPATIBLE_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")
OPENAI_COMPATIBLE_EMBEDDING_MODEL = os.getenv("OPENAI_COMPATIBLE_EMBEDDING_MODEL", "text-embedding-v4")
OPENAI_COMPATIBLE_EMBEDDING_DIMENSIONS = int(os.getenv("OPENAI_COMPATIBLE_EMBEDDING_DIMENSIONS", "1024"))
...
def rerank_results_with_xinference(query: str, results: List[Dict], top_k: int = 5) -> List[Dict]:
    """
...
def search_milvus_real(query: str, num_results: int = 5) -> List[Dict[str, str]]:
    ...
    try:
        ...
        for hits in results:
            for hit in hits:
                ...
                initial_results.append({
                    ...
                    'url': entity.get('url', ''),
                    'snippet': entity.get('content', '')[:300] + '...' if len(entity.get('content', '')) > 300 else entity.get('content', ''),
        ...
        if EMBEDDING_PROVIDER == "xinference" or check_xinference_rerank_available():
            search_results = rerank_results_with_xinference(query, initial_results, num_results)
        else:
            # 如果没有重排序，直接返回向量搜索结果
...
def check_milvus_connection() -> bool:
    ...
    try:
        ...
        # 检查集合是否存在
        has_collection = utility.has_collection(MILVUS_COLLECTION)
...
def check_xinference_connection() -> bool:
    ...
    try:
        ...
        # 检查所需的模型是否可用
        embedding_available = any(model["model_name"] == XINFERENCE_EMBEDDING_MODEL for model in models)
        rerank_available = any(model["model_name"] == XINFERENCE_RERANK_MODEL for model in models)
...
def launch_xinference_models():
    ...
    try:
        ...
        try:
            client.launch_model(
                model_name=XINFERENCE_EMBEDDING_MODEL,
        ...
        try:
            client.launch_model(
                model_name=XINFERENCE_RERANK_MODEL,
                model_type="rerank"
            )
